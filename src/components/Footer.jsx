...
const Footer = () => {
  ...
  const footerLinks = {
    ...
    services: [
      { label: 'Laundry Service', path: '/services' },
      { label: 'Dry Cleaning', path: '/services/dry-cleaning' },
      { label: 'Express Wash', path: '/services/express' },
      { label: 'Pickup & Delivery', path: '/services/delivery' },
    ],
    support: [
      ...
      <div className="container mx-auto px-6 pt-16 pb-8">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-3 group mb-6">
              <div className="relative">
                ...
                <div className="relative w-12 h-12 bg-gradient-to-br from-teal-400 to-teal-600 rounded-xl flex items-center justify-center shadow-lg shadow-teal-500/30 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
            ...
            <p className="text-slate-400 mb-6 leading-relaxed max-w-sm">
              Your trusted laundry service partner. Fresh clothes, delivered to your door with care and convenience.
            </p>
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-10 h-10 rounded-lg bg-slate-800/50 border border-slate-700/50 flex items-center justify-center text-slate-400 hover:text-teal-400 hover:bg-slate-800 hover:border-teal-500/50 transition-all hover:scale-110"
            ...
            <ul className="space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.path}>
                  <Link
                    to={link.path}
                    className="text-slate-400 hover:text-teal-400 transition-colors text-sm inline-flex items-center group"
                  >
        ...
        <div className="border-t border-slate-800/50 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-slate-500 text-sm">
            © {currentYear} LaundryMart. All rights reserved.
          </p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              {['Visa', 'Mastercard', 'PayPal'].map((method) => (
                <div
                  key={method}
                  className="px-3 py-1.5 bg-slate-800/50 border border-slate-700/50 rounded text-slate-400 text-xs font-medium"
                >
                  {method}
                </div>
              ))}
